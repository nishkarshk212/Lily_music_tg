...
async def init():
    ...
    # ✅ Try to fetch cookies at startup (skip if API keys are configured)
    if not (config.API_KEY and (config.API_URL or config.VIDEO_API_URL)):
        try:
        ...
        LOGGER("AnnieXMedia").info("🔑 Usɪɴɢ API ᴋᴇʏ ғᴏʀ YᴏᴜTuʙᴇ ᴅᴏᴡɴʟᴏᴀᴅs (ᴄᴏᴏᴋɪᴇs ɴᴏᴛ ʀᴇǫᴜɪʀᴇᴅ)")
