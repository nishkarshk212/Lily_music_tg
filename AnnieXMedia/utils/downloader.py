...
USE_NUBCODER_API = bool(API_KEY and (API_URL or VIDEO_API_URL))
...
def get_ytdlp_base_opts() -> Dict[str, object]:
    opts = {
        ...
        # Add user agent and headers to avoid 403 errors
...
async def api_download_audio(link: str) -> Optional[str]:
    ...
    if not vid:
        return None
    # Try new API format first (http://api.nubcoder.com)
    try:
        session = await get_http_session()
...
async def api_download_video(link: str) -> Optional[str]:
    ...
    try:
        session = await get_http_session()
        # New API: /info?token=API_KEY&q=VIDEO_URL
        api_url_new = f"{VIDEO_API_URL}/info?token={API_KEY}&q=https://www.youtube.com/watch?v={vid}"
        async with session.get(api_url_new, timeout=aiohttp.ClientTimeout(total=60)) as r:
            if r.status == 200:
                data = await r.json()
                # Check if new API format succeeded
